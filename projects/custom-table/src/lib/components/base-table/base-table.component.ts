...
export class BaseTableComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Column#showColumnOption set to false(default)
  ...
  @ViewChild('checkAll', {static: false}) public checkAll: Checkbox
        ...
        if(this._updateColumnFilter){
          ...
          results.forEach((item) => {
            ...
            if(columns[item.colIdx].columnFilter != undefined){
              ...
              cr.instance.subComponents = columns[item.colIdx].columnFilter.subComponents;
      ...
      this.bodyCellDirs.changes.subscribe(val => {
        if(this._updateBodyCellComponents){
          ...
          results.forEach(item => {
            ...
            if(columns[item.colIdx].bodyCell.component != undefined){
              const cf = this.cfr.resolveComponentFactory(
              ...
              // console.log('row idx')
              // console.log(item.rowIdx)

              if(columns[item.colIdx].bodyCell != undefined){
                cr.instance.config = columns[item.colIdx].bodyCell.config;
                cr.instance.field = columns[item.colIdx].bodyCell.field;
  ...
  private initCRSEvents(){
    ...
    this._subs.push(
      this._onClearFiltersEvent.subscribe(r => {
        ...
        if(this.config.processClearFilterEvent != undefined){
          this.config.processClearFilterEvent(r, this);
        }
      })
    )
  ...
  public exportButton(typ: ExportType){
    ...
    // if(this.config.columnHeaderConfig != undefined){
    //   this._initColumnOptions.forEach(item => {
    //     this.config.columnHeaderConfig.selectedValues.forEach(t => {
    //       if(item == t){
    //         fields.push(item);
    //       }
    //     })
