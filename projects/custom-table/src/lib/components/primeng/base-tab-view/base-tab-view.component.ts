...
export class BaseTabViewComponent implements OnInit, AfterViewInit, OnDestroy {
  private openedPanelIdxs: number[] = [];
  private panelCrs: ComponentRef<RowExpansionItemsI>[] = [];
  ...
  private initTabView(){
    this.panels.forEach((item) => {
      if(
        ...
        this.openedPanelIdxs.push(item.tabIdx)
  ...
  public change(event: any){
    ...
    for(let i = 0; i < this.openedPanelIdxs.length; i++){
      if(this.openedPanelIdxs[i] == event.index){
        found = true;
        break
      }
    }
    ...
    if(!found){
      ...
      this.openedPanelIdxs.push(event.index);
  ...
  public ngOnDestroy(){
    ...
    for(let i = 0; i < this.panelCrs.length; i++){
      this.panelCrs[i].destroy();
    }
